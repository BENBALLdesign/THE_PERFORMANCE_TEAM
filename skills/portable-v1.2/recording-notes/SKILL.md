...
Portable runtime: retain `../local_runtime.py` and read `../README.md`. Run the script by absolute path from any working directory. `--config` paths inside the JSON resolve relative to that JSON; the automatically discovered `../local-settings.json` is independent of the launch directory. Use `--check-runtime` to inspect dependency discovery. `--profile auto|low-memory|balanced|throughput`, `--memory-reserve-gb`, `--tool-dir`, `--model-dir`, and `--python-packages` are reusable command-line options. By default the runtime selects a locally cached model and conservative thread counts based on available memory. Sources and speech jobs remain serial. Explicit model and thread options override these defaults. Smaller models require careful review of technical terms and numbers. No automatic model download is permitted.
