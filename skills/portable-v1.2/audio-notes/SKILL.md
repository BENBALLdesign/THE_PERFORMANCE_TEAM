...
Portable runtime: retain `../local_runtime.py` and read `../README.md`. Run the script by absolute path from any working directory. `--config` paths inside the JSON resolve relative to that JSON; the automatically discovered `../local-settings.json` is independent of the launch directory. Use `--check-runtime` for dependency discovery. `--profile auto|low-memory|balanced|throughput`, `--memory-reserve-gb`, `--tool-dir`, `--model-dir`, and `--python-packages` are reusable command-line options. Default thread and cached-model choices adapt to available memory; explicit `--threads` and `--model` override them. One audio job runs at a time within the exporter. Avoid concurrent exporter invocations on low-memory machines. Verify technical terminology and numerical values from smaller speech models against the recording.
